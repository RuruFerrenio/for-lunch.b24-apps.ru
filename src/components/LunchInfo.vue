...
async function loadSettings(): Promise<void> {
  ...
  try {
    ...
    // Значения по умолчанию, если настройки не заданы
    const defaultStartTime = startTimeStr || '13:00'
    const defaultEndTime = endTimeStr || '14:00'

    lunchForm.startTime = defaultStartTime
    lunchForm.endTime = defaultEndTime

    formData.value.defaultLunchTime.startTime = stringToTime(defaultStartTime)
    formData.value.defaultLunchTime.endTime = stringToTime(defaultEndTime)
    ...
    // Устанавливаем значения по умолчанию при ошибке загрузки
    lunchForm.startTime = '13:00'
    lunchForm.endTime = '14:00'
    formData.value.defaultLunchTime.startTime = stringToTime('13:00')
    formData.value.defaultLunchTime.endTime = stringToTime('14:00')
  }
}
...
onMounted(async () => {
  if (typeof BX24 !== 'undefined' && BX24.init) {
    ...
    // Если BX24 не определен, устанавливаем тестовые значения для разработки
    lunchForm.startTime = '13:00'
    lunchForm.endTime = '14:00'
    formData.value.defaultLunchTime.startTime = stringToTime('13:00')
    formData.value.defaultLunchTime.endTime = stringToTime('14:00')
